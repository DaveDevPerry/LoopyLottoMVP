...
setTimeout(() => {
	...
	// check data has been input by the user
}, 1000);
function validateForm() {
	const inputUsername = document.querySelector('#username-input').value;
...
function validateUser(username, password) {
	// get array of obj - usernames
	console.log(username);
	console.log(password);

	const arr = getLogins(userData);
	console.log(arr);
	// check i
	arr.forEach((user) => {
		if (user.username === username && user.password === password) {
			...
			console.log('detals not recognised');
		}
	});
}

function getLogins(userData) {
	let loginArr = [];
	userData.data.user.forEach((user) => {
